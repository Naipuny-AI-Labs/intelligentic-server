...
const deleteAllOnBoardUsers = async (): Promise<any> => {
    try {
        // Use delete with empty where clause to delete all records
        await onBoardUserRepository.delete({})
        return { message: 'All users deleted successfully' }
    } catch (error) {
        throw new InternalError(
            StatusCodes.INTERNAL_SERVER_ERROR,
            `Error: onBoardUserService.deleteAllOnBoardUsers - ${getErrorMessage(error)}`
        )
    }
}
